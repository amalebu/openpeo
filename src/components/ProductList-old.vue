...
<template>
  <div class="flex flex-wrap my-4 divide-y divide-slate-200 px-1 pb-24 gap-2">
    <div class="px-4 py-4 w-full" v-for="product in productList.products" :key="product.id">
      <div class="h-full flex items-start justify-center text-left relative gap-4">
        <div class="flex-grow pt-2 flex-1">
            <div class="flex flex-wrap overflow-scroll item-center gap-2 w-full mb-2">
                <p class="bg-slate-200 text-slate-800 px-2 py-1 rounded-lg text-xs text-nowrap" v-for="tag in product.tags" :key="tag">
                    {{ tag }}
                </p>
              </div>
          ...
          <div class="flex flex-grow items-center gap-4">
            <div class="font-semibold text-base " :class="{'text-green-700': product.discountPercentage > 0}">Rp. {{product.price}}</div>
            <div class="text-sm line-through text-gray-500/50" v-if="product.discountPercentage > 0">Rp. {{product.basePrice}}</div>
          </div>
        </div>
        <div class="max-w-[20svw] min-w-[115px] relative">
          <p class="absolute top-2 -right-1 bg-red-500 text-slate-100 px-3 py-1 text-xs text-nowrap rounded-l-lg" v-if="product.discountPercentage > 0">
